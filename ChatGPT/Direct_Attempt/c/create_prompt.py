    ...
    for day in range(1, 26):
        ...
        if os.path.exists(file_a):
            ...
            with open(os.path.join(output_dir, f"prompt_{day_str}a.txt"), "w", encoding="utf-8") as f:
                f.write(base_prompt.replace("{puzzle_text}", desc_a).replace("Target language: Python", "Target language: C"))
        else:
        ...
        if os.path.exists(file_b):
            ...
            with open(os.path.join(output_dir, f"prompt_{day_str}b.txt"), "w", encoding="utf-8") as f:
                f.write(base_prompt.replace("{puzzle_text}", desc_b).replace("Target language: Python", "Target language: C"))
        else:
            print(f"Note: {day_str}b.txt not found — skipping b.txt")

    return "Prompt files created successfully in the 'prompts/c' directory."

base_dir = "/home/scriptie/Scriptie/ChatGPT/c"
print("Creating C prompts from aoc_puzzles...")
