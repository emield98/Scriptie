...
void resize_grid(char grid[MAX_ROWS][MAX_COLS], int new_rows, int new_cols) {
    char new_grid[MAX_ROWS][MAX_COLS];
    for (int i = 0; i < new_rows; i++) {
        for (int j = 0; j < new_cols; j++) {
            if (grid[i / 2][j / 2] == '#') {
                new_grid[i][j] = '#';
            } else if (grid[i / 2][j / 2] == 'O') {
                new_grid[i][j] = '[]';
...
int main() {
    ...
    // Resize the grid for the larger warehouse
    int new_rows = MAX_ROWS * 2;
    int new_cols = MAX_COLS * 2;
    resize_grid(grid, new_rows, new_cols);
