    ...
    for (int i = 0; i < n_antennas; i++) {
        for (int j = i + 1; j < n_antennas; j++) {
            if (antennas[i].freq == antennas[j].freq) {
                ...
                if (abs(dx) == 2 * abs(dy)) {
                    int x1 = antennas[i].x + dx / 2;
                    int y1 = antennas[i].y + dy / 2;
                    if (is_valid(x1, y1, max_x, max_y) && !visited[x1][y1]) {
                        visited[x1][y1] = 1;
                    }
                }
                if (abs(dy) == 2 * abs(dx)) {
                    int x2 = antennas[i].x + dx / 2;
                    int y2 = antennas[i].y + dy / 2;
                    if (is_valid(x2, y2, max_x, max_y) && !visited[x2][y2]) {
                        visited[x2][y2] = 1;
                    }
                }
            }
