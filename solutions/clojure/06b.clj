...
(defn next-turn [^Grid grid [^vec2 coord dir]]
  (loop [coord (move dir 1 coord)]
    (if-let [c (grid coord nil)]
      (if (= \# c)
        [(move dir -1 coord) (turn-right dir)]
        (recur (move dir 1 coord)))
      nil)))

(defn in-cycle? [^Grid grid [^vec2 coord dir]]
  (loop [node [coord dir]
         visited? #{[coord dir]}]
...
(defn part-2 [lines]
  (let [grid (lines->grid lines)
    ...
    (->> (dissoc visited? start)
         (map (fn [[obst-coord dir]]
                (let [start (move dir -1 obst-coord)]
                  (in-cycle? (assoc grid obst-coord \#) [start dir]))))
         (filter identity)
         count)))
