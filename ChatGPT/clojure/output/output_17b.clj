...
(defn parse-input [lines]
  (let [program (->> (drop 4 lines)
                     (mapcat #(str/split % #","))
                     (map #(Long/parseLong (str/trim %)))
                     vec)]
    {:B 0 :C 0 :program program}))
...
(defn run-program [program A]
  (loop [ip 0, reg {:A A :B 0 :C 0}, out []]
    (if (>= ip (count program))
      out
...
(defn part [lines]
  (let [{:keys [program]} (parse-input lines)
        goal program]
    (->> (iterate inc 1)
         (some (fn [a]
                 (when (= (run-program program a) goal)
                   a))))))
